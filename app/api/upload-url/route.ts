...
// Returns { clientToken, uploadUrl } — Blender uses these to PUT the file
// directly to Vercel Blob (never passes through our serverless function,
// so there's no timeout or size limit from our side).
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const clientToken = await generateClientTokenFromReadWriteToken({
      ...
      pathname,
      onUploadCompleted: {
        // Vercel calls this webhook after the upload finishes
    ...
    // Blender will PUT to:  https://blob.vercel-storage.com  (handled by Vercel)
