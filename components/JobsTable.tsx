...
// ---------------------------------------------------------------------------
// Column definition  (widthClass maps to .col-w-* in globals.css)
// ---------------------------------------------------------------------------
interface Column {
...
function formatDate(iso: string): string {
  try {
    // timeZone: 'UTC' prevents SSR↔client mismatch when server TZ ≠ browser TZ
...
// Linter requires ARIA boolean attrs to be static string literals, so we
// render two separate <button> elements and toggle between them.
// ---------------------------------------------------------------------------
...
function ColumnsPopover({ visible, onToggle }: ColumnsPopoverProps) {
  ...
  return (
    <div className="relative">
      {/* Static aria-expanded literals — linter requires non-expression values */}
      {open ? (
...
// Linter disallows expression values, so each sort state is a separate element.
// ---------------------------------------------------------------------------
interface ThProps {
  col: Column
  sortKey: SortKey
...
function Th({ col, sortKey: sk, sortDir: sd, onSort }: ThProps) {
  ...
  const icon = isSortable
    ? <SortIcon active={isActive} dir={isActive ? sd : 'asc'} />
    : null
  const handleClick = isSortable ? () => onSort(col.key as SortKey) : undefined

  // Static string literals for aria-sort — no expressions allowed by linter
  if (isActive && sd === 'asc') {
    return (
      <th scope="col" className={cls} aria-sort="ascending" onClick={handleClick}>
        {col.label}{icon}
      </th>
    )
  }
  if (isActive && sd === 'desc') {
    return (
      <th scope="col" className={cls} aria-sort="descending" onClick={handleClick}>
        {col.label}{icon}
      </th>
    )
  }
  return (
    <th scope="col" className={cls} onClick={handleClick}>
      {col.label}{icon}
    </th>
  )
}
    ...
    case 'preemptible':
      return (
        <span className={[
          ...
          aria-label={job.preemptible ? 'Yes' : 'No'}>
          {job.preemptible ? '✓' : '—'}
        </span>
      )
    case 'created':
      return (
        <time dateTime={job.created} className="text-gray-400 font-mono text-xs">
...
export default function JobsTable({ jobs }: JobsTableProps) {
  ...
  // project, title, progress are hidden by default to reduce horizontal crowding
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((job) =>
      Object.values(job).some((v) => String(v).toLowerCase().includes(q))
    )
  }, [jobs, search])

  const sorted = useMemo(
    () => sortJobs(filtered, sortKey, sortDir),
  ...
  const handlePageSize = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setPageSize(Number(e.target.value) as PageSize); setPage(1)
  }, [])
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map((job) => (
                <tr key={job.id} className="jobs-tbody-row">
                  {visibleColumns.map((col) => (
                    <td key={col.key}
                      className={['jobs-td', col.align ?? ''].join(' ')}>
